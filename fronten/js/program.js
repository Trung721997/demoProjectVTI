$(function() {
    ...
    $(".header").load("header.html", function() {
        document.getElementById("fullNam-login").innerHTML = storage.getItem("FULL_NAME");
        if (storage.getItem("ROLE") == "Manager") {
            // User 
            document.getElementById("viewListDepartments").style.display = "block";
...
function fillDepartmentToTable() {
    departments.forEach(function(item, index) {
        $('tbody').append(
            '<tr>' +
            '<td><input class="checkbox-der-item" id="checkbox-' + index + '" type="checkbox" onClick="onChangeCheckboxItem()"></td>' +
            '<td class="table-name">' + item.name + '</td>' +
            '<td class="table-author">' + item.author.fullName + '</td>' +
            '<td class="table-cretarDate">' + item.createDate + '</td>' +
            '<td>' +
...
// lưu lại tên cũ để khi update tên mới trùng tên cũ thì báo update thành công
var oldName;
